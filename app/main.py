...
ACC-007 audit: on startup we verify every APIRouter (except public whitelist)
has the get_current_user dependency at router level — not endpoint level.
...
# ── Import all models BEFORE create_all (SQLAlchemy needs them in scope) ─────
...
async def lifespan(app: FastAPI):
    """Startup: migrate + seed. Shutdown: nothing needed (sync engine)."""
...
# ── Public tracking (ACC-008 whitelist) ───────────────────────────────────────
@app.get("/api/v1/tracking/{tracking_number}", tags=["tracking"])
def public_tracking(tracking_number: str, db: Session = None):
...
# ACC-007: PUBLIC routes first (no auth dep)
app.include_router(auth_router)                  # /api/v1/auth — login, refresh, reset

# ACC-007: PROTECTED routes (all have Depends(get_current_user) at router level)
app.include_router(protected_auth_router)         # /api/v1/auth — logout, /me
app.include_router(users_router)                  # /api/v1/users
app.include_router(roles_router)                  # /api/v1/roles
app.include_router(companies_router)              # /api/v1/companies, /branches, /departments, /teams, /approvals
app.include_router(customers_router)              # /api/v1/customers, /suppliers
app.include_router(cargo_router)                  # /api/v1/shipments, /bags, /tracking
app.include_router(stock_router)                  # /api/v1/products, /stock-items, /reservations
app.include_router(orders_router)                 # /api/v1/orders, /exceptions
app.include_router(infra_router)                  # /api/v1/service-tickets, /solar-projects, /client-assets
app.include_router(commissions_router)            # /api/v1/commissions/*
app.include_router(finance_router)                # /api/v1/finance/*
